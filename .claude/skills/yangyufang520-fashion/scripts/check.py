...
# 实测：16 篇范文 0 处，被作者点名的低级句（版型一宽／带子一系／纱一飘）全在这个模子里。
# 一 后面必须是单字的动作或性状（宽/系/飘/提/挎），排除计数与虚词用法
# （一次/一遍/一回/一旦/一直/一点/一些/一路/一带/一片/一层/一半/一定/一样/一起），
# 否则"接了一次，……就够了"这种流水句会被误伤。
QUIP = r"[^,.]{1,6}一(?![次遍回旦直点些路带片层半定样起个种边天年角身件条双])[^,.]{1,2}[,][^,.]{0,8}就"
